...
window.Vaadin.Flow['_vaadintheme_v14-chart-test_globalCss'] = window.Vaadin.Flow['_vaadintheme_v14-chart-test_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_v14-chart-test_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_v14-chart-test_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_v14-chart-test_globalCss'].push(target);
  }
  if (!document['_vaadintheme_v14-chart-test_componentCss']) {
    
    document['_vaadintheme_v14-chart-test_componentCss'] = true;
  }
